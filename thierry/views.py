...
from .serializers import ClientPaymentSerializer, ManWithdrawSerializer
...
from .models import ClientPayment, ManufacturerWithdrawal
...
@api_view(['POST'])
def ClientPaymentCreate(request):
...
@api_view(['GET'])
def ClientPaymentList(request):
    payments = ClientPayment.objects.all()
    serializer = ClientPaymentSerializer(payments, many=True)
    return Response(serializer.data)


#All payments per client
@api_view(['GET'])
def ClientPayments(request, pk):
    client = Client.objects.filter(id = pk)
    payments = ClientPayment.objects.get(client=client)
    serializer = ClientPaymentSerializer(payments, many=True)
    return Response(serializer.data)


@api_view(['GET'])
def ClientPaymentDetail(request, pk):
    payment = ClientPayment.objects.get(id=pk)
    serializer = ClientPaymentSerializer(payment, many=False)
    return Response(serializer.data)
...
#Manufacturer Withdrawal views
@api_view(['POST'])
def ManufacturerWithdrawalCreate(request):
    serializer = ManWithdrawSerializer(data=request.data)
...
#All general withddrawals
@api_view(['GET'])
def ManufacturerWithdrawalList(request):
    withdrawals = ManufacturerWithdrawal.objects.all()
    serializer = ManWithdrawSerializer(withdrawals, many=True)
    return Response(serializer.data)


#All withdrawals per manufacturer
@api_view(['GET'])
def ManufacturerWithdrawalsRequested(request, pk):
    manufacturer = Manufacturer.objects.get(id = pk)
    withdrawals = ManufacturerWithdrawal.objects.filter(manufacturer=manufacturer, payment_requested=True)
    serializer = ManWithdrawSerializer(withdrawals, many=True)
    return Response(serializer.data)

#All withdrawals per manufacturer
@api_view(['GET'])
def ManufacturerWithdrawalsPaid(request, pk):
    manufacturer = Manufacturer.objects.get(id = pk)
    payments = ManufacturerWithdrawal.objects.filter(manufacturer=manufacturer, paid=True)
    serializer = ClientPaymentSerializer(payments, many=True)
    return Response(serializer.data)

@api_view(['GET'])
def ManufacturerWithdrawalDetail(request, pk):
    withdrawal = ManufacturerWithdrawal.objects.get(id=pk)
    serializer = ManWithdrawSerializer(withdrawal, many=False)
    return Response(serializer.data)


@api_view(['PUT'])
def ManufacturerWithdrawalUpdate(request, pk):
    withdrawal = ManufacturerWithdrawal.objects.get(id=pk)
    serializer = ManWithdrawSerializer(instance=withdrawal, data=request.data)
