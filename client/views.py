...
from .serializers import ClientSerializer,RequestSerializerPolite,RatingSerializer, RequestSerializer2
    ...
    for key, value in file_data.items():
        if isinstance(value, list) and len(value) == 1:
            if key in file_fields:
                ...
                file_data[key] = upload_response['secure_url']
...
def requestUpdate(request, pk):
    prod_request = Request2.objects.get(id=pk)
    serializer = RequestSerializerPolite(instance=prod_request, data=request.data)

    if serializer.is_valid():
        print('attempting to save')
        serializer.save()
        print('saved changes')
        print(Request2.objects.get(id=pk))
    else:
